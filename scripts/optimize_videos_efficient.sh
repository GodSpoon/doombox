...
# Pre-processes videos using NVENC hardware acceleration with parallel processing
# Optimizes videos in place and moves originals to backup directory
...
MAX_JOBS=$(($(nproc) / 2))  # Use half CPU cores to avoid overwhelming the GPU
...
update_progress() {
    local completed=$(find "$PROGRESS_DIR" -name "*.completed" 2>/dev/null | wc -l)
    local failed=$(find "$PROGRESS_DIR" -name "*.failed" 2>/dev/null | wc -l)
    local active=$(find "$PROGRESS_DIR" -name "*.active" 2>/dev/null | wc -l)
...
# Function to optimize a single video with enhanced NVENC settings
optimize_video() {
    local input_file="$1"
    local job_id="$2"
    ...
    # Mark job as active
    touch "$PROGRESS_DIR/${job_id}.active"
    
    local start_time=$(date +%s)
    
    # Enhanced NVENC settings for maximum efficiency while maintaining quality
    ...
    if [[ $encode_result -eq 0 && -f "$temp_output" ]]; then
        ...
        if [[ $output_size -gt 0 ]]; then
            ...
            # Calculate stats and save to progress file
...
for file in "${video_files[@]}"; do
    ...
    while [[ $(find "$PROGRESS_DIR" -name "*.active" 2>/dev/null | wc -l) -ge $MAX_JOBS ]]; do
        update_progress
        sleep 0.5
    done
...
while [[ $(find "$PROGRESS_DIR" -name "*.active" 2>/dev/null | wc -l) -gt 0 ]]; do
    update_progress
    sleep 1
done
...
if [[ $COMPLETED_FILES -gt 0 ]]; then
    ...
    find "$PROGRESS_DIR" -name "*.completed" -exec cat {} \; | while read -r line; do
...
if [[ $FAILED_FILES -gt 0 ]]; then
    ...
    find "$PROGRESS_DIR" -name "*.failed" -exec cat {} \; | while read -r line; do
